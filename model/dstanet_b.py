        ...
        if self.use_spatial_att:
            ...
            if self.att_s:
                ...
                attention = attention + self.tan(
                    torch.einsum('nscu,nscv->nsuv', [q.mean(-2), k.mean(-2)]) / (self.inter_channels)) * self.alphas
            if self.glo_reg_s:
        ...
        if self.use_temporal_att:
            ...
            if self.att_t:
                q, k = torch.chunk(self.in_nett(z).view(N, 2 * self.num_subset, self.inter_channels, T, V), 2,
                                   dim=1)  # nctv -> n num_subset c'tv
                attention = attention + self.tan(
                    torch.einsum('nsct,nscq->nstq', [q.mean(-1), k.mean(-1)]) / (self.inter_channels)) * self.alphat
            if self.glo_reg_t:
...
if __name__ == '__main__':
    ...
    net = DSTANet_B(config=config)  # .cuda()
