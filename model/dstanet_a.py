...
class STAttentionBlock(nn.Module):
    def __init__(self, in_channels, out_channels, inter_channels, num_subset=3, num_node=25, num_frame=32,
        ...
        if use_spatial_att:
            atts = torch.zeros((1, num_subset, num_frame, num_node, num_node))
            ...
            if att_s:
                ...
                self.alphas = nn.Parameter(torch.ones(1, num_subset, 1, 1, 1), requires_grad=True)
            if glo_reg_s:
                self.attention0s = nn.Parameter(torch.ones(1, num_subset, num_frame, num_node, num_node) / num_node,
                                                requires_grad=True)
        ...
        if use_temporal_att:
            attt = torch.zeros((1, num_subset, num_frame, num_frame, num_node))
            ...
            if att_t:
                ...
                self.alphat = nn.Parameter(torch.ones(1, num_subset, 1, 1, 1), requires_grad=True)
            if glo_reg_t:
                self.attention0t = nn.Parameter(torch.zeros(1, num_subset, num_frame, num_frame, num_node) + torch.eye(num_frame).unsqueeze(-1),
                                                requires_grad=True)
        ...
        if self.use_spatial_att:
            ...
            if self.att_s:
                ...
                attention = attention + self.tan(
                    torch.einsum('nsctu,nsctv->nstuv', [q, k]) / (self.inter_channels)) * self.alphas
            if self.glo_reg_s:
                attention = attention + self.attention0s.repeat(N, 1, 1, 1, 1)
            attention = self.drop(attention)
            y = torch.einsum('nctu,nstuv->nsctv', [x, attention]).contiguous() \
        ...
        if self.use_temporal_att:
            ...
            if self.att_t:
                q, k = torch.chunk(self.in_nett(z).view(N, 2 * self.num_subset, self.inter_channels, T, V), 2,
                                   dim=1)  # nctv -> n num_subset c'tv
                attention = attention + self.tan(
                    torch.einsum('nsctv,nscqv->nstqv', [q, k]) / (self.inter_channels)) * self.alphat
            if self.glo_reg_t:
                attention = attention + self.attention0t.repeat(N, 1, 1, 1, 1)
            attention = self.drop(attention)
            z = torch.einsum('nctv,nstqv->nscqv', [y, attention]).contiguous() \
...
if __name__ == '__main__':
    ...
    net = DSTANet_A(config=config)  # .cuda()
